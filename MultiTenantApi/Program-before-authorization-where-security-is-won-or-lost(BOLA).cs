...
//// Program.cs — MultiTenantApi.Secure (variant)
//// Human-readable formatting of the second blob you pasted (kept as-is, just prettified).
...
//        var hardening =
//            builder.Configuration.GetSection("TokenHardening").Get<TokenHardeningOptions>()
//            ?? new();
...
//                // {clientId} (GUID) — helps tooling like Postman
//            }
//            .Where(x => !string.IsNullOrWhiteSpace(x))
//            .ToArray(),
...
//app.UseAuthentication(); // recommended: AFTER UseAuthentication so ctx.User is populated.
...
//// NOTE: This blob uses the older QueryAsync signature (no tenantId param).
//// =====================================================
//app.MapGet(
//    "/raw-data",
//    async (
...
//        return Results.Ok(new { items = dto, count = dto.Count });
//    })
//.RequireRateLimiting("exports")
//.RequireAuthorization(AuthzPolicies.ReportsReadPolicyName)
//.WithOpenApi();
...
//        return RateLimitPartition.GetChainedLimiter(
//            key,
//            _ => new[]
//            {
